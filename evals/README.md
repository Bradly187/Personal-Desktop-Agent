...
| `retrieval` | does the retriever rank the known-target chunk in top-K? (**model-free scoring**, live chroma index) | `retrieval_synthetic` | **MRR** (Hit@5 reported) |
...
- **`retrieval_synthetic` → MRR over a self-filtered suite (a regression gate,
  not an absolute benchmark).** Cases are generated by
  `scripts/generate_retrieval_eval_data.py`, which keeps only questions the
  current retriever already answers at rank ≤ 10 — so the suite starts healthy
  by construction and exists to alarm on decay (embedder/index silently broken,
  chunking regressions). Targets match on `(file, name)` hit metadata; deleted
  files surface as errors, and the suite should be regenerated (command in its
  header comment) when errors exceed ~10% of n.
