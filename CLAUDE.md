...
## Current Status — Phases 1–6 + Sprints A–C/5–7/G1–G5/N/O/P/Q + gaze removal + AIOS alignment + goal sessions + magnetic cursor/gravity + agent-orchestration hardening + RAG/KB remediation + audit hash-chain + Gmail OAuth + skill model/breadth + proactivity + audio/voice pipeline + eval harness (2026-06-15)

> **Schema fact (authoritative, 2026-06-15):** `agent.db` = **42 tables** at `PRAGMA user_version = 7` (`storage/db.py`, lines 113–719); `AnalyticsDB` (DuckDB) holds the **3** `benchmark_*` tables. Older "Done" entries below state the table count *as of their own date* — they are historical, not current. Docs claiming 29/30/38/40/41 tables are stale.
...
- Cloud path: Anthropic API (`anthropic` SDK) via `_CloudInference` in `hybrid_coordinator.py`, model `claude-haiku-4-5` (8/8 accuracy on voice misrecognitions); 10s timeout circuit-breaker → CLARIFY. The dev-domain cloud path (`CloudDevAgent`) uses `claude-opus-4-8`.
- **Cloud backend selection (`core/cloud_backend.py`).** Both cloud consumers build their client through `resolve_backend()`, which picks the backend in one place: **direct Anthropic** (`ANTHROPIC_API_KEY`) by default, or **Claude in Amazon Bedrock** when an Amazon Bedrock API key (`AWS_BEARER_TOKEN_BEDROCK`) is set. Bedrock uses the documented bearer-token Mantle path — the ordinary `Anthropic`/`AsyncAnthropic` client pointed at `https://bedrock-mantle.{region}.api.aws/anthropic` with the Bedrock key as `api_key` — so the request shape (`messages.create`/`.stream`) is identical on both backends. Model ids are remapped to the `anthropic.`-prefixed Mantle ids (`anthropic.claude-haiku-4-5`, `anthropic.claude-opus-4-8`). Region: `DA_BEDROCK_REGION` → `AWS_REGION` → `us-east-1`. Force a backend with `DA_CLOUD_BACKEND=bedrock|anthropic`. A missing credential raises a clear, actionable message (degrades to CLARIFY, not a raw SDK traceback). (Previously migrated off the legacy AWS Bedrock `InvokeModel`/AgentCore path; this is the newer Messages-API Bedrock endpoint.)
