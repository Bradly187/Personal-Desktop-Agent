...
- **Chat active-directory switching + file-context attachments (2026-06-28).** The desktop chat (`--chat`, `core/chat_server.py` :8770 + `web_client_chat/`) can now switch the DevAgent's working project mid-session and attach `.pdf`/`.png`/`.svg` files as context. **Active dir (browse + confirm):** WS `set_active_dir`/`list_dirs` → `HybridCoordinator.set_active_directory(path, confirm=)` re-scopes `CommandExecutor._writable_roots` (via `set_active_root`/`add_writable_root`) **and** `DevAgent._repo_root` (via `set_repo_root`). A path already in scope activates immediately; a **new** root returns `confirm_required` and is appended to the allowlist only on an explicit `confirm=true` (AGENTS.md #7 — never bypasses `_path_in_scope`). **Attachments:** `POST /upload` (multipart, `.pdf/.png/.svg` only, 25 MB cap, traversal-guarded) stores under `<active_root>/.chat_uploads/`; `inference/attachments.extract_attachment` (pure, never raises) turns each into **text** (pdf via pypdf/pdfplumber; svg XML fallback) or an **image** (png b64; svg rasterized via PyMuPDF — no cairosvg dep). The chat server threads `attachment_context` + the first `attachment_image_b64` into `Command.params`; the coordinator dev pre-gate forwards them to `DevAgent.handle(attachment_context=…, screenshot_b64=…)` (image → resident qwen3-vl, **no new model**, AGENTS.md #6). No schema change (#1), off the 60 Hz path (#2). **Deferred:** cloud-DevAgent attachment path, multi-image, image-only-PDF OCR. Spec: `specs/chat-context-attachments/`. Tests: `test_attachments.py` (10) + `test_chat_attachments.py` (9) + `test_chat_attachments_server.py` (9).
...
- **Cloud plan routing is experimental and OFF by default (`DA_CLOUD_PLAN`).** Addresses the VRAM-budget thrash: `DevAgent.plan_and_run` generates its executable step array via `ModelRouter.infer(domain="plan")` → `qwen3-coder:30b` (~18 GB), which can't co-reside with the resident command+general models on 32 GB and forces a ~50–60 s evict/reload swap. When ON, `inference/cloud_plan_router.CloudPlanRouter` (a **transparent proxy** wrapping `ModelRouter`, installed in `main.py` between `ModelRouter()` and `DevAgent(...)`) intercepts **ONLY `domain="plan"`** and serves it from **Claude Sonnet 4.6 via Bedrock** (forced tool-use `emit_plan` whose `input_schema` IS `model_router._PLAN_JSON_SCHEMA` + the same `_PLAN_PROMPT` — so cloud output parses byte-compatibly with the local path; thinking disabled since forced tool_choice + extended thinking aren't combinable). Every other domain/method delegates unchanged via `__getattr__`. **Pure inference — executes no desktop/file/shell action** (the agentic loop stays local). **Fail-safe (AGENTS.md #4):** missing SDK/credential/grant, network/API error, or schema-invalid output → transparent fallback to the local `infer("plan")` (the local plan model is NEVER removed — offline planning still works); OFF or no credential = byte-identical legacy. **Privacy:** the plan context carries the user's source (repo facts, git working-tree diff, RAG, paths), so goal+context are scrubbed via `ContentFilter` before egress and a **critical** finding (private/API/cloud key) forces the LOCAL path. Cost recorded to the `inferences` ledger (backend `bedrock`, no schema change). Distinct from `--cloud-dev-agent` (which routes dev *answer* queries cloud-side and runs NO execution). Spec: `specs/cloud-plan-routing/`. Tests: `test_cloud_plan_router.py` (10).
