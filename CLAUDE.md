...
- **DevAgent context compaction (`DA_COMPACT_CONTEXT`, default on).** The `_replan`/`_reflect` prompts feed executed-step outcomes back to the planner. With compaction on (default), each step is summarised to its salient line (first line for a success; the error tail for a failure) and the whole executed-history block is held under a ~2000-token budget — every FAILED step is kept (the planner needs the error to recover), the oldest SUCCESSFUL steps are elided first (with a "… N earlier steps elided" marker). Full step output is ALWAYS preserved in `agent_steps.result`; compaction only trims the prompt. `DA_COMPACT_CONTEXT=0` restores the prior fixed-char-cut behaviour (clean A/B). Helpers `_summarize_result`/`_fit_history_to_budget` in `inference/dev_agent.py`; tests `tests/test_dev_agent_compaction.py` (12).
