...
- `docs(db)` `45c0283` — 9 Mermaid ER diagrams added to `docs/architecture/database-design.md` §11 + §12 two-tier note; fixed stale "embedding always NULL/Jaccard" and table-count claims (agent.db = **29 tables**, not 32).
...
- `db.py` — +1 table: `ipad_logs`; +1 method: `log_ipad_events(session_id, entries)`; total is now **29 AgentDB tables** (the 3 `benchmark_*` tables in `db.py` belong to the DuckDB `AnalyticsDB`, not `agent.db`)
...
| `storage/db.py` | `AgentDB` (aiosqlite, 29 tables, all pipeline writes; versioned via `PRAGMA user_version`) + `AnalyticsDB` (DuckDB, 3 benchmark tables); MiniLM semantic retrieval; gesture velocity + voice + iPad log tables |
