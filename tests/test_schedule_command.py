...
async def test_schedule_creates_scheduled_goal(tmp_path):
    ...
    try:
        ...
        assert any(s["goal"] == "brief me" for s in await db.goals.list_schedules())
    finally:
        await db.close()
...
async def test_event_rule_command(tmp_path):
    ...
    try:
        ...
        assert any(r["topic_pattern"] == "email.arrived" for r in await db.events.list_event_rules())
    finally:
        await db.close()
...
async def test_list_and_cancel_all(tmp_path):
    ...
    try:
        ...
        assert await db.goals.list_schedules() == []
    finally:
        await db.close()
