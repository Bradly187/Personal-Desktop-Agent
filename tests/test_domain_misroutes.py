...
Verifies AgentDB.routing.get_domain_misroutes (per-domain routed vs corrected → rate) and
...
async def _seed_domain(db, sid, domain, n, n_corrected):
    ...
    for k in range(n):
        ...
        if k < n_corrected:
            await db.commands.mark_command_corrected(cid, "FIXED it")
...
async def test_get_domain_misroutes_rate():
    ...
    try:
        ...
        rows = {r["domain"]: r for r in await db.routing.get_domain_misroutes()}
...
async def test_analyzer_flags_and_logs_high_rate_domain():
    ...
    try:
        ...
        logged = await db.routing.get_recent_adaptation_log("misroute:code")
        assert logged, "expected a misroute:code adaptation_log row"
        assert not await db.routing.get_recent_adaptation_log("misroute:command")
    finally:
        await db.close()
...
async def test_analyzer_respects_min_data_floor():
    ...
    try:
        ...
        assert not await db.routing.get_recent_adaptation_log("misroute:math")
    finally:
        await db.close()
