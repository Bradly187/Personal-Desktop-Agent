...
async def test_tick_promotes_and_kicks_drainer():
    ...
    db.enqueue_scheduled_goal = AsyncMock()
    dev = MagicMock(); dev.drain_goal_queue = AsyncMock()
    ps = ProactiveScheduler(db, dev_agent=dev)
    assert await ps._tick(1000.0) == 1
    db.promote_due_goals.assert_awaited_with(1000.0)
    db.enqueue_scheduled_goal.assert_not_awaited()   # one-shot: no re-lay
...
async def test_tick_relays_recurrence():
    ...
    db.enqueue_scheduled_goal.assert_awaited()
    assert db.enqueue_scheduled_goal.await_args.kwargs["execute_at"] == 1060.0


async def test_tick_skips_during_flare():
    db = MagicMock(); db.promote_due_goals = AsyncMock(return_value=[])
    sched = MagicMock(); sched.dev_paused = True
    ps = ProactiveScheduler(db, scheduler=sched)
    assert await ps._tick(1000.0) == 0
    db.promote_due_goals.assert_not_awaited()   # flare: never even queried
