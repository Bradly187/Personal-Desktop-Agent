...
async def _seed_run(db) -> int:
    ...
    return await db.runs.start_agent_run("escalation test", "plan", "model")
...
async def test_insert_and_get_pending(tmp_path):
    ...
    esc_id = await db.sagas.insert_escalation(
...
async def test_get_pending_newest_first_and_limit(tmp_path):
    ...
    run_id = await _seed_run(db)
    for i in range(4):
        await db.sagas.insert_escalation(run_id, f"goal {i}", "max_steps")
        await asyncio.sleep(0.01)  # distinct timestamps for ORDER BY ts

    items = await db.sagas.get_pending_escalations(limit=2)
    assert len(items) == 2
...
async def test_count_pending(tmp_path):
    db = await _open_db(tmp_path)
    run_id = await _seed_run(db)
    assert await db.sagas.count_pending_escalations() == 0
    await db.sagas.insert_escalation(run_id, "g1", "max_replans")
    await db.sagas.insert_escalation(run_id, "g2", "max_steps")
    assert await db.sagas.count_pending_escalations() == 2
    await db.close()
...
async def test_resolve_all(tmp_path):
    ...
    cleared = await db.sagas.resolve_escalations(status="acknowledged")
    assert cleared == 2
    assert await db.sagas.get_pending_escalations() == []
    assert await db.sagas.count_pending_escalations() == 0
    await db.close()
...
async def test_resolve_single_by_id(tmp_path):
    db = await _open_db(tmp_path)
    run_id = await _seed_run(db)
    first = await db.sagas.insert_escalation(run_id, "g1", "max_replans")
    await db.sagas.insert_escalation(run_id, "g2", "max_steps")

    cleared = await db.sagas.resolve_escalations(status="dismissed", escalation_id=first)
    assert cleared == 1
    remaining = await db.sagas.get_pending_escalations()
...
async def test_closed_db_noops():
    ...
    assert await db.sagas.insert_escalation(1, "g", "max_replans") is None
    assert await db.sagas.get_pending_escalations() == []
    assert await db.sagas.count_pending_escalations() == 0
    assert await db.sagas.resolve_escalations() == 0
...
async def test_record_escalation_insert_failure_falls_back_to_sidecar(tmp_path):
    ...
    db = MagicMock()
    db.available = True
    db.sagas.insert_escalation = AsyncMock(side_effect=RuntimeError("disk full"))
    agent = _make_agent(db)
    agent._escalation_sidecar_path = tmp_path / "esc.jsonl"
...
def test_review_queue_with_pending_items():
    db = MagicMock()
    db.available = True
    db.sagas.count_pending_escalations = AsyncMock(return_value=2)
...
def test_review_queue_empty():
    db = MagicMock()
    db.available = True
    db.sagas.count_pending_escalations = AsyncMock(return_value=0)
    db.sagas.get_pending_escalations = AsyncMock(return_value=[])
    c = _coord(db)
...
def test_clear_review_queue():
    db = MagicMock()
    db.available = True
    db.sagas.resolve_escalations = AsyncMock(return_value=3)
    c = _coord(db)
    res = _route(c, "Clear review queue.")
    assert res["action"] == "AGENT_ESCALATIONS_CLEAR"
    assert res["count"] == 3
    db.sagas.resolve_escalations.assert_awaited_once_with(status="acknowledged")
