...
async def test_list_and_cancel_schedule(tmp_path):
    db = await _db(tmp_path)
    try:
        gid = await db.enqueue_scheduled_goal("water plants", execute_at=5000.0)
        assert any(s["id"] == gid for s in await db.list_schedules())
        assert await db.cancel_schedule(gid) is True
        assert all(s["id"] != gid for s in await db.list_schedules())
        assert await db.cancel_schedule(gid) is False  # already cancelled
    finally:
        await db.close()
...
async def test_recurrence_carried_on_promote(tmp_path):
    ...
    try:
        ...
        await db.enqueue_scheduled_goal("morning brief", execute_at=1.0, recurrence=rec)
        promoted = await db.promote_due_goals(2.0)
...
async def test_event_rules_crud(tmp_path):
    db = await _db(tmp_path)
    try:
        rid = await db.insert_event_rule(
        ...
        await db.touch_rule_fired(rid, 1234.0)
        r = next(r for r in await db.list_event_rules() if r["id"] == rid)
        assert r["fire_count"] == 1 and r["last_fired_at"] == 1234.0
        assert await db.cancel_event_rule(rid) is True
        assert all(r["id"] != rid for r in await db.list_event_rules(enabled_only=True))
    finally:
        await db.close()
